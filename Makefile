...
DEPLOY_HOST := $(DEPLOY_USER)@putget.qwasa.net
...
help:
	...
	-@echo ' make tea'
	-@echo ' ---'
	-@echo ' make remote_install_user DEPLOY_HOST=root@putget.qwasa.net # root access required'
...
remote_copy_files: ## copy files (to deploy host)
	...
	-scp -r ./deploy ./misc/www "$(DEPLOY_HOST):$(DEPLOY_PATH)"
	-scp -r ./_keys/htpasswd "$(DEPLOY_HOST):$(DEPLOY_PATH)/htpasswd"
...
create_keys:
	...
	ssh-keygen -t rsa -b 4096 -f ./_keys/id_rsa_putget -N "" -C "putget"
	[ -f ./_keys/htpasswd ] || \
	(htpasswd -cb ./_keys/htpasswd "$(HT_USER)" "$(HT_PASSWD)" && \
...
remote_sudo_install: create_keys ## create service user (@deploy host)
	...
	"
	-scp -r ./_keys/id_rsa_putget.pub "$(DEPLOY_HOST):$(DEPLOY_PATH)/.ssh/authorized_keys"
	ssh $(DEPLOY_HOST) sudo "\
